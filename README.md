...
## 編集室（EDITOR'S ROOM）— モノローグ下書きとSNS候補

毎朝の生成時に、5本の候補をもとにした**ニュースレター用モノローグの下書き**（英語・日本語）と**SNS投稿候補**も自動生成され、アプリの「EDITOR'S ROOM」タブに表示されます。

- **英語モノローグ**: Substack用。スタンダップコメディ構成（掴み→5本のくだり→締め）で、「東京でアメリカのニュースを読んで頭を抱えている男」のペルソナ。末尾に出典リスト（Today's Docket）付き
- **日本語モノローグ**: note用。同じ人物が日本の読者向けに話し直した版
- **Substack Notes候補**（英語8本）と**X候補**（日本語8本・135字以内）: 1タップでコピーして投稿できます

**重要**: これはあくまで下書きです。掴みと締めを自分の言葉に書き直し、事実確認をしてから投稿してください。SubstackはAI検出ツール（Pangram）を導入しており、無編集のAI文をそのまま出すのはリスクがあります。「声」の最終パスだけは人間の仕事です。

- 生成に失敗した場合も5本の候補は通常どおり出力されます（編集室が空になるだけ）
- 無効化したい場合は Actions の Variables に `EDITORIAL_ENABLED = 0` を追加
- モノローグだけ別モデルにしたい場合は `EDITORIAL_MODEL` を設定（既定はSATIRE_MODELと同じ）
