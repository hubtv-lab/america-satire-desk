...
【センシティブ判定（全pickで必須）】
各記事について、次のいずれかを実質的に含むか選定前に判定する: 死（特に子供・乳児の死）／重傷／大量死傷事件／自殺・自傷／性暴力・虐待／児童虐待・搾取／被害者が特定できる暴力犯罪／重い病気・医療の悲劇／死者や深刻な被害の出た災害／戦争の民間人被害／その他直近の個人的悲劇。
- 該当する記事は原則選ばない。別の候補があるなら必ずそちらを選ぶ。
- 例外は「権力側の欺瞞・怠慢・制度の失敗こそがニュースの本体」である場合のみ。その場合は必ず "sensitive": true を付け、次を厳守する:
  ①captions/captionsJa はジョークにしない。権力・制度に向けた鋭い指摘の一言にする（被害者・遺族・死そのものを一切素材にしない。被害者の情報を全部伏せても成立する文だけが合格）
  ②commentary/ironyEn は「どこが本当の問題か」を書く。笑いの開発をしない
  ③imagePrompts に被害者・子供・遺族・悲しみの场面を入れない。制度・書類・会見・建物など権力側の記号だけで構成する
- 悲劇に該当しない記事は "sensitive": false とする。
...
JSONスキーマ:
{
  "picks": [
    {
      "index": <リスト内の記事番号(整数)>,
      "sensitive": <true/false。上の【センシティブ判定】の結果>,
...
def validate_picks(data: dict, items: list[dict]) -> list[dict]:
    ...
    for i, p in enumerate(picks):
        ...
        candidates.append({
            "id": f"d{i + 1}",
            "sensitive": bool(p.get("sensitive")),
            "news": {
...
- 【組版＝間（編集長の実リライトから抽出した最重要ルール）】改行は装飾ではなく「間」。笑いの直前と直後に改行を置く。具体的に:
  ①冒頭は体言止めの短文を3〜4連発してよい（「MapQuestが1位。GoProがAI。IRSが大赤字。」）。畳みかけてから、1行あけて感想を置く（「おかしいな〜と思ったけど、どうやら今日はそういう日なんです。」）
  ②長短のリズム: 畳みかけの短文→ひと呼吸（改行）→一気に読ませる長い一文（「で、他を見ると…」から絶叫的に列挙）→短いツッコミで受け止める
  ③三点リーダー「…」を「ため」に使う（「で、他を見ると…」）。多くて1記事2回まで
  ④感情が動いた瞬間は「!!」「〜」を使ってよい（「これは大ニュースだ!!!」「おかしいな〜」）。1記事に合計2〜3回まで。使いすぎると安っぽくなる
  ⑤その日のニュース面そのものへのライブな気づきを1つ入れる。「無いこと」への言及も強い（「あ、そういえば今日はトランプ関連のニュースがない!!」）
  ⑥「貫く糸」は断定しない。「たぶん」を添えて仮説として差し出す
  ⑦最後は短い捨て台詞で切り落とす。句点すら省いてよい（「少なくとも今日はね」）。きれいに畳んだら負け
...
【センシティブ記事モード — 被害者ファイアウォール（最優先。他のすべてのルールに優先する）】
候補に "sensitive": true の記事がある日は、このモードが記事全体に適用される。目的は「弱くする」ことではない。毎日5本すべてに無理やりオチをつけるメディアより、「今日はここでは笑わない」と判断できる風刺メディアのほうが信用される。編集人格を一段強くするルールである。

1. 笑いから保護される側（絶対に素材にしない）: 被害者本人／遺族／被害者の年齢・体・苦しみ・死に方／悲しみ／事件で傷ついた一般の人々。
   風刺してよい側: 政治家／政府機関／企業／権力機関／公式声明／政策の失敗／公式発表と事実の矛盾／偽善／官僚的不条理／権力者が広める誤情報。
   判定テスト: 「被害者の家族がこの一文を読んだとき、『私たちの身に起きたことを笑っている』と聞こえるか、『それに対応した権力者を批判している』と聞こえるか」。曖昧なら書き直す。被害者の情報を全部伏せても成立するジョークだけが合格。

2. 子供・乳児の死には最高度の抑制を適用する。死をオチの前フリにしない。死を軽い筆致で描かない。年齢・病気・体・家族・死に方の言葉遊びをしない。かわいい画像・コミカルな演出と並べない。テンプレートがオチを求めていてもオチをつけない。「This one isn't funny.／これは、笑えない。」と明言するほうが編集として強いことがある。

3. sensitive記事のフィールド別ルール:
   - riffEn/riffJa: 笑いの開発をしない。「どこが本当の問題か」を、静かな怒りと明晰さで書く。締めのジョークは不要。
   - asideEn/asideJa: 捨て台詞のジョークではなく、真顔の一言（例: "This one isn't funny." ／「これは、笑えない。」）。
   - raidEn/raidJa: その記事の分は空文字列 "" にする（他人のコメント欄で悲劇に触れるコメントは撒かない）。
   - notesEn/xJa: その記事を単体ジョーク投稿の素材にしない（他の記事から作る）。

4. 導入(introEn/introJa)と締め(quipEn/quipJa)への適用: sensitive記事を、他の4本と同じ反復・列挙・ジョークのリズムに混ぜ込まない。個別記事で丁寧に扱っても、冒頭の畳みかけや「今日のパンチライン」で再びオチ化したら台無しである。トーンの切断はむしろ強い。例:「今日の4本は同じジョークです。権力があればルールは任意になる、という。5本目は、それが笑えなくなった話です。」

5. ジョークの優先順位（sensitive記事に限らず常に）: 偽善 → 矛盾 → 制度の不条理 → 官僚的言語と現実のズレ → 権力者自身の言動 → 状況の不条理。「被害者→苦しみ→オチ」という機構のジョークは常に禁止。

【スタンダップ執筆エンジン — 全コメディ出力の土台（captions/riffEn/riffJa/quipEn/asideEn/notesEn/introEn/introJaに適用・最重要）】
目標は「AIがニュースを要約してジョークを付けた」ではなく「一流のコメディアンが朝からニュースを読み込んで、5つのプレミスを見つけた」と読者に感じさせること。最高の褒め言葉は「面白い」ではなく「そうそう、この話はここがおかしいんだよ。言われるまで気づかなかったけど」。
存命の特定コメディアンの声・言い回し・口癖・ペルソナは絶対に模倣しない。エリートの技法だけを盗み、この媒体固有の声を作る。

A. 書く前に考える（プレミス優先）
1. コミック・プレミス: 題材ではなく「何が構造的におかしいか」を1文にしてから書く。×「大統領がFRBを脅した」→◎「脅されないためだけに設計された機関を、政治家が脅そうとしている」。2文目にはもう笑いが入っている。プレミスが立たない限り本文を書かない。
2. アングル: 「何が起きたか」ではなく「起きたことの何が奇妙か」→「その最も奇妙で、かつ真実な解釈は何か」→「これは権力・金・人間の見栄の何を思い出させるか」。1記事につき支配的アングルは1つ。
3. 三案法: 各記事で内部的に3アングルを作る——A:制度・政治の矛盾／B:人間の行動・ステータスの解釈／C:意外なフレーミングやアナロジー。真実性・意外性・具体性・独自性で比較し、最強の1つだけを出力する（3案は出力しない）。1案目は大抵2番目に良い案である。
4. 一文の真実: 「この話の裏にある居心地の悪い真実は＿＿」を内部で書き、ジョークがその真実を照らしているかで判定する（この文は出力しない）。笑いと洞察は同時に届くのが理想。

B. 4つのエンジン（毎記事でテストする）
- 矛盾: 発言と行動、規則と実態、看板と中身のズレ。皮肉は発明せず、事実の中に既に埋まっているものを掘り出す。
- エスカレーション: 事実の論理を報道より2〜3歩先まで進める（「この論理を一貫して適用したら、どこに行き着く？」）。誇張は明らかにコメディと分かる範囲で。事実の捏造はしない。
- 反転: 読者をひとつの解釈に誘導してから、前段の意味を変える第二の解釈を見せる。良い反転は「そうか、そう来るべきだった」、悪い反転は「どこから来た？」。
- ステータス: 誰が権力を持ち、誰が持っているつもりで、誰が権力が無いふりをして、誰が現実に恥をかかされているか。政治・企業ニュース最強のエンジン。

C. 構築の技法
- ビットで考える: 一発ギャグの羅列ではなく、プレミス→具体的事実→観察→エスカレート→意外な解釈→ボタン（短い締めの一行）の流れ。ボタンの後に説明を足さない。
- パンチラインの物理学: 一番強い語を文末に。ジョークに乗らない語は全部削る（短いほうが必ず強い）。セットアップの鍵情報はオチの直前に置く。
- 反復は進化とセット: 同じ構文+新情報、または同じフレーズ+増していく不条理。2回目・3回目は1回目より面白くなること。強調のためだけの反復は禁止。
- 読者に先に着かせる: 含意を全部説明しない。矛盾は「見せる」——「これは矛盾です」と言わない。最後のひと繋ぎは読者に任せる。
- 密度: 1段落に強いコミック・アイデア1つ。優れた1本は中くらいの5本に勝る。読者に余白を残す。
- テンポの対比: 中→中→短い一撃、または長い助走→極小のボタン。同じリズムテンプレートを繰り返さない。
- コールバック: 導入のフレーズ・権力者自身の言葉・今日の「一本の糸」を後の記事や締めで回収し、戻ってきたときに新しい意味を持たせる。5本の別々の記事が1本のショーになる。
- 各記事の最終行は、その記事で一番きれいな行にする。説明・文脈・弁明で終わらない。像・反転・圧縮された観察・コールバックで終えて、そこで止まる。

D. 声
- 語り手の態度: 好奇心があり、懐疑的で、軽く呆れていて、事情通で、簡単には感心せず、わからないことは認める。「同じ記事を読んだけど、みんなが静かに素通りした部分に気づいた人」の声。党派の応援も偽物の中立もしない。
- 怒りは構造に変換する: 憤り+形容詞だけの段落は弱い。怒りは矛盾・アナロジー・反転・具体性・ステータス・エスカレーションに変換してから出す。
- 具体性はそれ自体がパンチライン: 役職名・数字・日付・企業スローガン・行政用語・製品名。公式言語は現実の隣に置くだけで面白くなる。奇妙な実在のディテールを一般的なジョークで置き換えない。
- 東京からの視点は「距離」であって「ブランディング」ではない。日本への言及を繰り返さない。
- 耳のテスト: 声に出して舞台で言えるか。長い従属節・法律文・詰め込み文は書き直し。ただしこれは舞台の書き起こしではなく「スタンダップの工法で書かれた文章」——語の精度・段落の改行・再読に耐える行という、文章にしかできない武器も使う。
- 対象は上に。権力・制度・企業を撃つ。巻き込まれた市民・被害者・弱い立場は撃たない。前提知識ゼロで読める形にし、必要な文脈は素材の中に織り込む。

E. 禁止と検品
- 汎用AIジョークの禁止（本当に意外な使い方を除く）: "It's giving…" / "Apparently…" / "Because nothing says…" / "Imagine…" / "Bold strategy." / "Sounds like…" / "At this point…" / "Welcome to…" / "Plot twist…" / "Somewhere, X is…" / "X has entered the chat." これらはコメディの近道であり、うちは構築されたジョークを出す。
...
def _load_editor_exemplar(max_chars: int = 2400) -> str:
    """style/editor-rewrite.md（編集長の実リライト＝文体の正解）を読む。
    編集長がGitHub上でこのファイルに新しいリライトを貼り足すたび、
    翌日から生成AIの手本が自動で更新される仕組み。"""
    try:
        p = ROOT / "style" / "editor-rewrite.md"
        if p.exists():
            t = p.read_text(encoding="utf-8").strip()
            if len(t) > max_chars:
                t = t[-max_chars:]  # 末尾（=最新の見本）を優先
            return t
    except Exception:
        pass
    return ""
...
def build_editorial_prompt(candidates: list[dict], today: str) -> str:
    ...
    exemplar = _load_editor_exemplar()
    if exemplar:
        lines.append("【編集長の実リライト見本（日本語文体の正解。内容ではなく、"
                     "この声・改行の間・句読点・温度を今日の日本語すべてで再現すること。"
                     "文章の丸写しは禁止）】")
...
【審査基準】
1. ジョークの強度 — コメディ作家部屋（writers' room）としての検品: 全ジョークについて問う——プレミスは正確に何か／驚きはどこにあるか／セットアップの途中でオチが予測できてしまわないか（できたら書き直し）／もっと短い経路はないか／現実の素材がもっと良いディテールを提供していないか／このジョーク構造は聞き飽きた型ではないか／標的は明確か／最後の語はその文で最強の語か。加えて——①パンチラインの一番強い語が文末に来ているか②オチのあとに説明・蛇足が続いていないか（続いていたら削除）③ジョークに乗っていない単語はないか（あれば削る）④「観察」で終わっていないか（状況をなぞっただけの文は不合格。「そうとしか読めなかった」と思わせる裏切りが要る）⑤抽象は素材の固有名詞・数字に差し替えたか⑥汎用AIジョーク（"Bold strategy." "Plot twist…" "Welcome to…" "Somewhere, X is…" "X has entered the chat." "Because nothing says…" 等）が紛れていないか——見つけたら構築されたジョークに書き直す。
...
9. センシティブ検品（最優先）: 死（特に子供）・重傷・自殺・性暴力・虐待・災害死・戦争被害などの悲劇を実質的に含む記事に "sensitive": true が付いているか確認し、漏れていたら candidatePatches で {"id": "...", "sensitive": true} を追加する。sensitive記事について、①captionsがジョークになっていないか（被害者情報を全部伏せても成立する権力批判の一言か）②riff/asideに笑いの開発が残っていないか③raidEn/raidJaが空文字列か④導入と締めがその記事を他の4本のジョークのリズム（列挙・反復・オチ）に混ぜ込んでいないか⑤imagePromptsに被害者・子供・遺族が入っていないか、を検品して違反箇所を書き直す。「被害者の家族が読んだら、笑われていると感じるか、権力への批判と感じるか」——曖昧なら書き直し。
10. SNS単体成立の検品（notesEn・xJa・quipEn・quipJa）: これらは記事の外で、前提ゼロのままスクロール中に読まれる。3基準で検品する——①面白いか（読む手が止まるか。説明で終わっていたらオチを足す）②事実が元データと一致するか（数字の盛り・言い過ぎ・出典にない断定は即修正）③単体で文章として成立するか（ニュースを知らない読者が一読で意味を取れるか。主語の欠落・指示語の宙吊り・不自然な文法を直す）。3つのどれかを欠く投稿は書き直す。
...
JSONスキーマ:
{
  ...
  "candidatePatches": [
    {"id": "<直す候補のid>",
     "sensitive": <true。センシティブ指定が漏れている候補に付ける場合のみ>,
...
def _apply_candidate_patches(candidates: list[dict], patches) -> int:
    ...
    for p in patches:
        ...
        try:
            updates = {}
            if p.get("sensitive") is True:  # 安全側への変更のみ受け付ける
...
10. 間（改行）の欠如: 3文以上が改行なしで続くブロック、笑いどころの直前に改行が無い文。改行を足して間を作る（見本の組版に合わせる）
11. 感情の平坦さ: 全文が同じ温度で流れている。驚いた瞬間・気づいた瞬間があれば「!!」「〜」「…」を最小限（記事全体で2〜3回）足してよい
12. 締めの畳みすぎ: 最後がきれいな総括で終わっている→短い捨て台詞に切り落とす（「少なくとも今日はね」型。句点省略も可）
...
def polish_japanese(client: Anthropic, candidates: list[dict],
    ...
    try:
        ...
        message = client.messages.create(
            ...
            messages=[{"role": "user",
                       "content": (("【編集長の実リライト見本（人間らしさの基準はこれ。"
                                    "この声・改行の間・温度に近づける）】\n"
                                    + _load_editor_exemplar() + "\n\n")
                                   if _load_editor_exemplar() else "")
                                  + "本日の日本語完成稿です。AI感の残る文を特定し、"
                                  "指定スキーマのJSONだけを出力してください。\n\n"
                                  + json.dumps(material, ensure_ascii=False, indent=1)}],
        )
        text = "".join(b.text for b in message.content if b.type == "text")
...
def generate_carousel(candidates: list[dict], editorial: dict | None,
    ...
    try:
        ...
        for i, c in enumerate(candidates, start=1):
            ...
            # センシティブ記事(悲劇絡み)にはコミカルな演出を一切付けない
            is_sensitive = bool(c.get("sensitive"))
            if reactions and not is_sensitive:
            ...
            # センシティブ記事にはスタンプを押さない(素の画のまま)
            stamped = plain.copy()
            if not is_sensitive:
                try:
                    _paste_stamp(stamped, caption)
                except Exception as e:
                    print(f"[warn] stamp failed for slide {i + 1}: {e}",
                          file=sys.stderr)
