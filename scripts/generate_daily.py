...
【英語ジョークの職人ルール — スタンダップの現場基準（captions/riffEn/quipEn/asideEn/notesEnすべてに適用・最重要）】
アメリカのプロのジョーク作家（The Onion編集部、深夜番組の放送作家、スタンダップの教科書）が共有する原則。英語ネイティブの読者は無意識にこの基準で判定している。
1. パンチラインは最後の単語。文の一番意外な語・強い語を文末に置く（"punchline word last"）。オチのあとに説明・補足を一切足さない。オチのあとの一文は蛇足であり、笑いを殺す。
2. 経済性。ジョークに乗っていない単語は全部削る。同じオチなら短いほうが必ず強い。形容詞と副詞をまず疑う。
3. 意外だが、後から見れば必然。読者に予想させず、しかしオチを見た瞬間「そうとしか読めなかった」と思わせる（misdirection with logic）。状況をなぞっただけの文は「観察」であって「ジョーク」ではない。不合格。
4. セットアップの鍵情報はオチの直前に置く。セットアップとパンチの距離が近いほど笑いは強い。
5. 具体が抽象に勝つ。"old technology" ではなく "a server from 2004"。固有名詞・数字・日付は素材にある限り温存し、ジョークの弾として使う。
6. 態度（POV）。語り手が本当にそう感じている一言だけが刺さる。「うまいこと言おうとした文」より「本音がこぼれた文」。この記事の語り手は東京から見て本気で困惑している——その困惑を使う。
7. 内部で量産して選別する。各captionは頭の中で3案作り、最強の1本だけを出力する（3案は出力しない）。The Onionは週数百本の見出しから十数本しか使わない。あなたの1本目の案は大抵2番目に良い案である。
8. 対象は上に。権力・制度・企業を撃つ。巻き込まれた市民・被害者・弱い立場は撃たない。
9. 前提知識ゼロで読める。アメリカ内輪ネタの知識を要求しない。素材の中に文脈を織り込む。
10. 5本を通した設計: 後半ほどエスカレートさせ、quipEnで今日の「一本の糸」をコールバックとして回収すると、記事全体が1本のセットリストになる。
...
【審査基準】
1. ジョークの強度（スタンダップ現場基準で検品）: 「説明」で終わっているcaptionは、オチのある一言に書き直す。チェック項目——①パンチラインの一番強い語が文末に来ているか（来ていなければ語順を組み替える）②オチのあとに説明・蛇足が続いていないか（続いていたら削除）③ジョークに乗っていない単語はないか（あれば削る。短いほうが必ず強い）④意外性があるか（状況をなぞっただけの「観察」は不合格。読み終えた瞬間「そうとしか読めなかった」と思わせる裏切りが要る）⑤具体か（"old technology"型の抽象は素材の固有名詞・数字に差し替える）。
